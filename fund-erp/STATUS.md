...
4. **코덱스 2차 검증 잔여** — 2026-07-22 추가로 ⒁·⑿·⑷·⑻ 수정 완료(commit 00270a9). **남은 것(모두 위험 낮음~사용자결정)**: ⑼목적사업 분류(WELF_CATS)와 회계 계정 불일치(계정과목 확장은 사용자 결정 필요), ㉕객체 키 inline-JS 삽입(키가 push()/FUND-NNNN이라 실위험 낮음 — JSON 가져오기 키 검증 추가 시 해소).
