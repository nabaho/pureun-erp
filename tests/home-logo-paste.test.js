...
test('★★ 파일 고르기와 붙여넣기가 «같은 길»로 담는다', () => {
  ...
  /* ⚠ 2026-09-06 「여러장 한번에」로 파일 한 장을 살피는 일이 로고파일담기 로 옮겨졌다.
     파일 고르기·끌어다 놓기가 «그 하나»를 거쳐 로고담기 로 간다 — 길은 여전히 하나다. */
  assert.match(함수('로고파일담기'), /로고담기\(/, '★ 파일 한 장 담는 길이 끊겼다');
  ['addPartnerLogo', 'logoDrop'].forEach(f =>
    assert.match(함수(f), /로고여러장\(/, '★ ' + f + ' 가 딴 길로 담는다'));
  assert.match(함수('로고여러장'), /로고파일담기\(/, '★ 여러 장 담기가 딴 길로 간다');
  ...
  assert.ok(!/f\.name/.test(담기.replace(/사연/g, '')),
    '★ 남이 준 파일 이름을 주소로 쓴다');
  /* ⚠ 이름 짓기가 로고열쇠() 로 옮겨졌다 (2026-09-06) — 여러 장을 한꺼번에 담으면
     같은 밀리초에 둘이 만들어져 이름이 겹치고, 겹치면 뒤엣것이 앞엣것을 덮어
     로고 한 장이 «조용히» 사라진다. 그래서 이미 담긴 것과 견주게 됐다. */
  assert.match(담기, /로고열쇠\(\)/, '★ 이름을 짓는 곳을 안 거친다');
  const 열쇠 = 함수('로고열쇠');
  assert.match(열쇠, /'n' \+ Date\.now\(\)/,
    '★ 파일 이름을 우리가 짓지 않는다 — 붙여넣기에는 이름이 아예 없다');
  /* ⚠ 「있는것[ 이 소스에 있나」로 보면 안 된다 — 값을 «모으는» 줄에도 그 글자가 있어,
     비켜 가는 줄을 지워도 통과했다(2026-09-06 되돌림 검사가 잡았다).
     그래서 아래에서 «실제로 두 번 지어» 본다. */
  const 열쇠짓기 = (담긴것) => new Function('App', 'Date',
    함수('로고열쇠') + '\nreturn 로고열쇠;')(
      { partnerLogos: { 추가: 담긴것 } }, { now: () => 1700000000000 })();
  assert.equal(열쇠짓기([]), 'n1700000000000', '★ 이름을 시각으로 안 짓는다');
  assert.notEqual(열쇠짓기([{ srl: 'n1700000000000' }]), 'n1700000000000',
    '★★ 같은 밀리초에 담으면 이름이 겹친다 — 뒤엣것이 앞엣것을 덮어 로고 한 장이 조용히 사라진다');
  assert.notEqual(열쇠짓기([{ srl: 'n1700000000000' }, { srl: 'n1700000000000-2' }]),
    'n1700000000000-2', '★★ 세 장째가 두 장째를 덮는다');
});

/* ══════ 여러 장 한꺼번에 (대표 지시 2026-09-06 「여러장 한번에」) ══════ */
test('★★ 로고를 여러 장 «한꺼번에» 고르고 끌어다 놓을 수 있다', () => {
  /* 자문사를 수십 곳 고르면 로고도 그만큼 필요하다 — 한 장씩이면 그 수만큼 왕복한다 */
  const 창 = 함수('drawPartnerLogos');
  assert.match(창, /<input type="file" multiple/,
    '★★ 파일 고르기가 한 장만 받습니다');
  assert.match(창, /ondrop="logoDrop\(event\)"/, '★★ 끌어다 놓을 칸이 없습니다');
  /* ⚠ dragover 에서 막지 않으면 브라우저가 그림을 새 탭으로 열어 화면을 통째로 떠난다 */
  assert.match(창, /ondragover="logoDragOver\(event,true\)"/,
    '★★ 끌어 올린 것을 안 막습니다 — 브라우저가 그림을 열어 화면을 떠납니다');
  assert.match(함수('logoDragOver'), /preventDefault/, '★★ 기본 동작을 안 막습니다');
  assert.match(함수('logoDrop'), /preventDefault/, '★★ 놓을 때 기본 동작을 안 막습니다');
});

test('★★ 한 장이 잘못돼도 «나머지는 담는다» — 열 장을 통째로 물리지 않는다', () => {
  /* 한 장이 크다고 열 장을 다 물리면, 어느 것이 문제였는지 모른 채 처음부터 다시 고른다 */
  const s = 함수('로고여러장');
  assert.match(s, /try \{[\s\S]*?catch/, '★★ 한 장이 죽으면 거기서 멈춥니다');
  assert.match(s, /안된것/, '★ 못 담은 것을 안 모읍니다');
  assert.match(s, /된것\.length/, '★ 몇 장이 담겼는지 안 알려 줍니다');
  /* 어느 파일이 왜 안 됐는지 이름과 까닭이 함께 남아야 다시 넣을 수 있다 */
  assert.match(s, /f\.name \+ ' — '/, '★★ 못 담은 파일의 이름·까닭을 안 알려 줍니다');
  /* 몇 장째인지 보여야 한다 — 스무 장을 담는 동안 멈춘 줄 안다.
     ⚠ 「목록.length 가 소스에 있나」로 보면 for 문이 대신 걸린다(되돌림 검사가 잡았다).
       «세는 수를 화면에 적는가»를 본다. */
  assert.match(s, /알림\([^)]*i \+ 1/,
    '★★ 몇 장째를 담고 있는지 안 보여 줍니다 — 스무 장을 담는 동안 멈춘 줄 압니다');
});
