...
test('★★ 긴 설명은 «접어» 둔다 — 늘 펼쳐 두면 이 줄만 남의 세 배가 된다', () => {
  /* ⚠ 처음에 넉 줄을 통째로 펼쳐 두었다. 폰(375px)에서 이 줄 하나가 287px —
       이웃(125·106px)의 두 배 반이었고 화면 삼분의 일을 먹었다.
     ★ 접어도 글자는 화면 안에 있다(DOM). 그래서 위의 «안 열었다» 검사가 그대로 산다.
     ★ 밖에 남기는 것은 둘뿐이다: 무엇을 하는가, 그리고 «빈칸을 오해하지 말라».
       그 둘은 접으면 안 된다 — 접힌 경고는 없는 경고다. */
  const seg = switchRow();
  assert.match(seg, /<details class="msmore">/, '긴 설명을 안 접었습니다');
  const i = seg.indexOf('<details');
  const shown = seg.slice(0, i);
  assert.match(shown, /안 ?열었다/, '「안 열었다」 경고를 접어 버렸습니다 — 접힌 경고는 없는 경고입니다');
  const brs = (shown.match(/<br>/g) || []).length;
  assert.ok(brs <= 2, '접기 밖에 ' + (brs + 1) + '줄이 남았습니다 — 두 줄까지입니다');
});
