...
test('★ 이름(글 제목)은 채우지 않는다 — 카드가 「권형하대표」 한 덩어리라 덮으면 직책이 사라진다', () => {
  assert.equal(F.MEMBER_FIELDS.some(f => f.key === '이름'), false,
    '★ 이름을 채우려 한다 — 글 제목이 「권형하」인지 「권형하대표」인지 아직 모른다');
  const { doc, fields } = 사람화면();
  F.fillMemberFields(doc, { 이름: '딴사람', 경력사항: '새 경력' });
  assert.equal(fields[0].value, '박성수', '★ 제목 칸을 건드렸다');
});
