...
const 최소 = [
  { i: 2, ko: '상호',       px: 250, why: '「태양농산 주식회사 ⚠ 사업자등록증 +2」 (몸)' },
  { i: 3, ko: '폴더',       px: 161, why: '「📁 통합기술보호지원단」 134 + 여백 27 (몸)' },
  { i: 4, ko: '유형',       px: 80,  why: '머리글 「유형」+화살표+깔때기 (머리 — 몸보다 크다)' },
  { i: 5, ko: '사업자번호', px: 126, why: '「134-86-05772」 102 + 여백 24 (몸)' },
  { i: 6, ko: '가진 것',    px: 214, why: '42+62+나머지 76 + 여백 24 (몸)' },
  { i: 7, ko: '담당',       px: 88,  why: '넉 자 이름 64 + 여백 24 (몸)' }
];
...
test('★ 그래도 좁은 창(1280 − 옆줄 240 = 1040)을 넘지 않는다', () => {
  /* 위 최소들을 다 더해도 여기 들어가야 한다 — 안 들어가면 «칸을 줄일 것»이 아니라
     «칸을 없앨 것»을 먼저 물어야 한다. 지금은 34+46 을 더해 1004 다. */
  const at = SRC.indexOf('<colgroup><col style="width:34px">');
  const seg = SRC.slice(at, SRC.indexOf('</colgroup>', at));
  const w = (seg.match(/width:(\d+)px/g) || []).map(x => Number(x.match(/\d+/)[0]));
  const sum = w.reduce((a, b) => a + b, 0);
  assert.ok(sum <= 1040,
    '★ 칸 폭 합이 ' + sum + ' 이다 — 1040 을 넘으면 좁은 창에서 좌우로 넘친다');
});
