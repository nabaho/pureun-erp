...
test('★★ 단추가 그 스위치에 매여 있다 — 켜면 «정말로» 다시 나온다', () => {
  ...
  assert.match(code, /CO_WEBSITE_FIND_ON && h\('button'/,
    '★★ 단추가 스위치와 이어져 있지 않습니다');
  /* 단추를 통째로 지워 놓고 스위치만 남기는 것도 막는다 */
  assert.match(code, /'🔍 찾기'\)/,
...
test('★★ 기능은 지우지 않았다 — 다시 켤 때 처음부터 만들지 않게', () => {
  ...
  assert.match(fn, /exports\.findCompanyWebsite/,
    '★★ 서버 함수를 지웠습니다 — 스위치만 켜서는 안 돌아갑니다');
  /* 부르는 자리도 남아 있어야 한다 */
  assert.match(code, /searchCompanyWebsite/,
    '★★ 화면에서 부르는 자리를 지웠습니다 — 스위치를 켜도 아무 일이 없습니다');
  assert.match(code, /findCompanyWebsite/,
    '★ 서버 주소를 지웠습니다');
});

test('★ 「홈페이지」 칸은 그대로 — 손으로 적는 것은 된다', () => {
  assert.match(code, /'홈페이지'\)/, '★★ 칸까지 없애면 홈페이지를 아예 못 적습니다');
  assert.match(code, /value:f\.website \|\| ''/, '★ 적은 값이 안 담깁니다');
});
