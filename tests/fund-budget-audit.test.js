...
test('통째로 덮어쓰는 단추는 두지 않는다', () => {
  ...
  /* ★ 2026-09-06 바로잡음 — 「되돌리기는 일부러 두지 않았다」는 «틀린 안내»였다.
     공용 부품 js/pu-backup.js 가 이미 매일 통째로 떠서 30시점을 보관하고,
     화면 왼쪽 아래 [백업·복구] 단추로 되돌린다. 앱은 사실대로 고쳐졌는데
     이 검사만 옛 «글자»를 붙들고 있어 멀쩡한 수정 때문에 깨졌다(CLAUDE.md).
     ⚠ 있는 기능을 «없다»고 하는 것이 더 나쁘다 — 쓸 수 있는 길을 스스로 막는다.
       보아야 할 것은 글자가 아니라 «되돌리는 길이 어디 있는지 알려 주는가»다. */
  assert.ok(/통째로 되돌리기[\s\S]{0,140}백업·복구/.test(SRC),
    '되돌리는 길이 어디 있는지 안 알려 준다 — 있는 기능을 못 쓰게 만든다');
