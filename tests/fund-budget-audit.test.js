...
test('통째로 덮어쓰는 단추는 두지 않는다', () => {
  ...
  /* ⚠ 2026-09-02 — 예전에는 `onclick="exportAll()"` 을 «글자 그대로» 찾았다.
     백업을 상단 ⚙〈백업·복구〉 창으로 옮기며 `onclick="closeM();exportAll()"` 이
     되자 그 자리에서 깨졌고, main 이 빨개져 모든 배포가 막혔다(CLAUDE.md 「검사를
     쓰는 규칙」). 지켜야 할 것은 «어떤 글자»가 아니라 「누를 수 있는 자리에서
     백업이 불린다」는 것이다. */
  assert.match(SRC, /onclick="[^"]*exportAll\(\)/,
    '백업을 부르는 손잡이가 화면에 없다 — 함수만 있고 누를 데가 없으면 아무 뜻이 없다');
});

test('백업은 메뉴 순서에 안 섞인다 — 메뉴가 아니라 도구다', () => {
  /* ⚠ 예전에는 `id="nav-backup"` 이 «있어야» 통과했다. 2026-09-02 백업이 메뉴에서
     아주 빠져 상단 ⚙ 로 옮겨지자 「백업 단추가 없다」로 깨졌다 —
     메뉴에서 빼는 것이 이 검사가 바라던 «바로 그것»인데도.
     그래서 「메뉴에 있으면 순서에 안 섞인다」로 고쳤다: 메뉴에 없으면 그대로 통과. */
  const i = SRC.indexOf('id="nav-backup"');
  if (i < 0) return;                 // 메뉴에서 아주 빠졌다 — 더 강하게 지켜진 셈이다
