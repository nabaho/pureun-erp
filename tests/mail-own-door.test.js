...
test('메일은 즐겨찾기 손잡이에 있고, 포털 첫 화면에는 «타일을 안 낸다»', () => {
  /* 2026-08-21 처음에는 문을 셋 냈다 — 바탕화면 아이콘·포털 타일·즐겨찾기 손잡이.
     같은 날 대표가 포털 타일만 물렸다: 「메일은 여기 있을 필요 없다, 기업정보함으로」.
     한 프로그램이 첫 화면에서 타일 둘을 차지해 목록만 길어졌기 때문이다.
     ⚠ 물린 것은 «포털 타일 하나»뿐이다 — 바탕화면 아이콘(별도 manifest)과 즐겨찾기
       손잡이는 전날 지시 그대로 남긴다. 셋 다 지우면 메일을 없앤 것이 된다. */
  assert.doesNotMatch(enter, /key:'mail'/,
    '★ 포털 첫 화면에는 메일 타일을 두지 않습니다 — 기업정보함 하나로 들어갑니다.');
  assert.match(enter, /key:'cards'[\s\S]{0,120}?메일/,
    '★ 그 대신 기업정보함 설명에 메일을 적어야 어디서 찾을지 압니다.');
