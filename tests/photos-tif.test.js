...
test('★★ 못 여는 까닭을 «그 파일에 맞게» 말한다 — 엉뚱한 안내는 없느니만 못하다', () => {
  ...
  /* ⚠ 2026-08-30 에 이 줄을 고쳤다. 예전에는 「높은 호환성」이라는 «말 자체»를 박아
     두었는데, 그날 규칙이 바뀌어 이 검사가 깨졌다 — 기능이 망가져서가 아니다.
     이제 HEIC 는 «안내하지 않고 우리가 바꾼다»(heic2any). 여기까지 오는 것은
     도구를 못 받았을 때뿐이다.
     그래서 「무슨 말을 하는가」가 아니라 «지켜야 할 것»만 본다:
       ① 아이폰 사진이라고 알아본다  ② 이미 찍힌 사진에게 「다시 찍어라」 하지 않는다 */
  const heic = c.cantOpenWhy({ name: 'IMG_0001.HEIC', type: '' });
  assert.match(heic, /HEIC|아이폰/, '★ 아이폰 사진인 줄 모르고 딴 말을 합니다:\n  ' + heic);
  assert.ok(!/다시 찍어/.test(heic),
    '★ 이미 찍힌 사진에게 「다시 찍어라」는 안내가 아닙니다:\n  ' + heic);
