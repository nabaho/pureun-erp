...
/* ══════════════════════════════════════════════════════════════════════════
   칸 통수 — 「누르면 줄어드는 숫자」 (2026-08-27 검토에서 나옴)
   ══════════════════════════════════════════════════════════════════════════
   ⚠ 예전에는 폴더를 아직 «안 읽었을 때만» 서버 셈을 쓰고, 읽고 나면 «읽어 온 줄»을
     세었다. 그래서 「1.자문사답변 404」가 그 칸을 누르는 순간 100 으로 줄었다.
     한 통도 사라지지 않았는데 숫자만 준 것이라, 보는 사람은 메일이 없어진 줄 안다.
   ★ 셀 수 있다 — 서버 폴더 통수에서 «밖으로 옮긴 것»을 빼고 «들어온 것»을 더한다.
     쪽지(_mbPut)는 한 쪽만 받아 오는 것이 아니라 통째로 있으므로, 이 셈은 얼마나
     읽어 왔는지와 «상관이 없다». */

test('★ 칸 통수는 읽어 온 양에 흔들리지 않는다 — 눌렀다고 400이 100으로 줄면 안 된다', () => {
  /* 서버는 「6.공공기관에 3통」이라고 적어 두었다. 그런데 우리가 읽어 온 것은 «한 통»뿐 —
     첫 쪽만 받아 온 상태다(실제로는 400 가운데 100 을 받아 온 모습). */
  const half = load({ msgs: { B_GONG: { '20': MSGS.B_GONG['20'] } } });
  assert.equal(half.mbBinCount(half.mbBinBy('B_GONG')).n, 3,
    '읽어 온 만큼만 세고 있습니다 — 누르면 숫자가 줄어 보입니다');

  /* 한 통도 안 읽어 왔을 때와 «같은 값»이어야 한다 */
  const none = load({ msgs: {} });
  assert.equal(none.mbBinCount(none.mbBinBy('B_GONG')).n, 3, '안 읽었을 때와 값이 다릅니다');

  /* 다 읽어 왔을 때도 같다 */
  const all = load();
  assert.equal(all.mbBinCount(all.mbBinBy('B_GONG')).n, 3, '다 읽었을 때와 값이 다릅니다');
});

test('★ 그러면서도 옮긴 것은 셈에 그대로 반영된다 — 안 그러면 숫자가 거짓말이 된다', () => {
  /* 한 통을 6.공공기관 → 1.자문사답변 으로 옮겼다. 읽어 온 것은 여전히 «한 통»뿐이다. */
  const c = load({ msgs: { B_GONG: { '20': MSGS.B_GONG['20'] } },
                   put: { 'B_GONG:20': 'B_JAMUN' } });
  assert.equal(c.mbBinCount(c.mbBinBy('B_GONG')).n, 2, '밖으로 옮긴 것이 안 빠졌습니다');
  assert.equal(c.mbBinCount(c.mbBinBy('B_JAMUN')).n, 2, '들어온 것이 안 더해졌습니다');

  /* ⚠ 옮겨 온 메일이 «안 읽어 온 폴더»에 있어도 세어야 한다.
       읽어 와야만 세면, 옮겨 놓고 다른 칸을 보다 오면 숫자가 달라진다. */
  const far = load({ msgs: {}, put: { 'B_INBOX:10': 'B_JAMUN' } });
  assert.equal(far.mbBinCount(far.mbBinBy('B_JAMUN')).n, 2,
    '안 읽어 온 폴더에서 옮겨 온 것을 못 셉니다');
});
