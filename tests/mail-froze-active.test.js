...
test('★★ 단추줄과 안내줄이 «한 덩이»로 붙박여 있다 — 내려가도 늘 그 자리', () => {
  const c = load();
  const h = c.mbListHtml();
  const i = h.indexOf('dm-froze');
  assert.ok(i > 0, '붙박이 덩이(.dm-froze)가 없습니다 — 목록을 내리면 단추가 사라집니다');
  /* 단추줄·안내줄이 «그 안»에, 목록은 «그 밖»에 있어야 한다 */
  const tools = h.indexOf('dm-tools'), hint = h.indexOf('dm-hint'), list = h.indexOf('dm-list');
  assert.ok(i < tools && i < hint, '단추줄·안내줄이 붙박이 덩이 밖에 있습니다');
  assert.ok(hint < list, '안내줄이 목록보다 아래에 있습니다');
  /* ⚠ 「안내줄 뒤 첫 </div>」를 보면 안 된다 — 그것은 «안내줄 자신»의 닫음이라
