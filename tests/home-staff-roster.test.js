...
test('★★ 「올림/안 올림」이 «구분 딱지와 같은 줄»에 있다 — 따로 두면 두 줄을 먹는다', () => {
  const s = 함수('memberEdit');
  /* 첫 딱지줄(.fldbar)이 끝나기 «전»에 올림/안올림 단추가 있어야 한다.
     ★ 값이 아니라 자리를 본다 — 라벨을 따로 세우면 라벨 한 줄 + 딱지 한 줄이 된다.
     ★ 「이름 줄」로 찾지 않는다 — 2026-09-03 에 이름 칸이 아래 줄(직책·글번호와 한 줄)로
       내려가고 이 줄의 라벨이 「구분」이 되었다. 지켜야 하는 규칙은 «구분 딱지와
       올림 딱지가 같은 한 줄에 있다»는 것이고, 그 줄의 이름이 무엇인지가 아니다. */
  const 줄시작 = s.indexOf('<div class="fldbar">');
  assert.ok(줄시작 >= 0, '★ 첫 딱지줄(.fldbar)을 못 찾았다');
  const 줄끝 = s.indexOf("</div>'", 줄시작);
  assert.ok(줄끝 > 줄시작, '★ 첫 딱지줄이 어디서 끝나는지 못 찾았다');
  const 한줄 = s.slice(줄시작, 줄끝);
  assert.match(한줄, /kindSet\(/, '★ 구분(노무사/직원) 딱지가 첫 줄에 없다');
  assert.match(한줄, /offSiteSet\(false\)/,
    '★★ 「올림」 단추가 그 줄 밖에 있다 — 라벨 한 줄 + 딱지 한 줄로 두 줄을 먹는다');
  assert.match(한줄, /offSiteSet\(true\)/, '★★ 「안 올림」 단추가 그 줄 밖에 있다');
