...
test('★ 기업 상세에서 탭 줄을 숨기지 않는다 — 숨기면 탭이 갈 곳이 없다', () => {
  ...
  /* ⚠ 2026-08-27 뒤집혔다 — 예전에는 «폴더를 안 골랐을 때» 머리줄을 접었다
     (2026-08-17, 「보여 줄 탭이 없어 빈 띠만 남는다」). 그 뒤 이 줄에 거래처·종료·
     번호없음·정보부족·고유번호증 단추가 들어와 더는 빈 띠가 아닌데, 규칙이 남아 있어
     «첫 화면에서 그 단추가 하나도 안 보였다». 대표 결정으로 걷어냈다.
     이제 기업 상세에서는 폴더를 골랐든 아니든 머리줄이 보인다. */
  assert.equal(/isCo && !state\.coFolder/.test(fn), false,
