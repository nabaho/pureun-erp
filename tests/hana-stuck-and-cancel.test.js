...
test('★★ 연결됐는데 한 건도 없을 때 «할 일»을 표가 바로 말한다', () => {
  /* 대표 2026-08-29: 「연결했다고 하는데 왜 안들어오나」.
     이 앱은 알림을 엿듣는다 — 알림은 지나가면 사라진다. 그래서 연결만 해서는
     «앞으로 올» 문자만 들어오고, 그동안 온 것은 앱의 주황 단추로만 들어온다.
     표가 「받은 문자 없음」이라고만 하면 고장인 줄 알고 엉뚱한 데를 뒤진다. */
  const chip = bare(cutBlock(ERP, 'function hanaStatChip(){'));
  const call = chipCalls(chip).filter((c) => /연결됨/.test(c) && !/lastOkAt\)\)/.test(c))
    .find((c) => /지난 문자 가져오기|받은 문자 없음/.test(c));
  assert.ok(call, '★ 「연결됐는데 한 건도 없다」 갈래를 못 찾았다 — 검사가 헛돌고 있다');
  assert.ok(call.indexOf('「지난 문자 가져오기」') >= 0,
