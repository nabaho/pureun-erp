...
test('★ 업로드는 한 자리뿐이다 — 그런데 언제나 닿을 수 있어야 한다', () => {
  /* 2단계에서는 이 검사가 «아직 걷지 마라»는 울타리였다. 사업장 줄을 접으면
     #dash-upload 가 사라져 업로드할 길이 드래그뿐이 되기 때문이다.
     3단계가 「① 넣기」를 만들어 그 까닭을 없앴다 — 이제 울타리를 반대로 세운다:
     한 자리뿐이되, ①을 눌러 «언제든» 그 자리에 닿을 수 있어야 한다. */
  assert.match(src, /id="upBtn"/, '업로드 단추가 사라졌습니다');
  assert.ok(!src.includes('id="dash-upload"'), '업로드가 아직 두 자리입니다');
  assert.match(src, /id="stepbar"/,
    '단계 막대가 없으면 ①로 돌아갈 길이 없어 업로드에 못 닿습니다');
  const a = src.indexOf('id="setup"');
  assert.ok(a >= 0 && src.slice(a, a + 1400).includes('id="upBtn"'),
    '업로드는 ① 넣기(설정 줄) 안에 있어야 합니다');
});
