/* 손을 댄 자리와 실제로 고쳐지는 자리가 «같은 자리»여야 한다
   ...
   ■ 왜
     사진에 걸어 둔 `max-height:100%` 의 100% 는 **어미 칸의 높이**를 뜻하는데
     그 칸의 높이가 auto 였다 — **정해지지 않은 높이의 100% 는 없는 것으로 친다.**
     높이 제한이 통째로 사라졌다. CSS 가 조용히 무시하는 자리라 화면만 봐서는 안 보인다.

   ■ 고침 셋 (셋이 함께 있어야 뜻이 있다)
     ① 칸에 «정해진 높이»(height:100%) — 그래야 100% 가 뜻을 갖는다
     ② 칸에 «사진의 본디 비» — 그래야 칸이 사진보다 넓거나 좁아지지 않는다
     ③ 사진은 칸을 «그대로 채운다» — max- 로 다시 재면 ①의 병이 되돌아온다

   ⚠ 2026-08-29 저녁, 편집이 가리기에서 갈라져 나오면서(대표 지시 「편집기능 분리」)
     이 자리는 **편집기의 판(#edWrap)**으로 옮겨졌다. 병도 고침도 같으므로 검사도 옮긴다.
     가리기(🔒 가리고 판독)의 옆 칸은 예전부터 맞게 돌고 있다(재 봤다: 440×330 = 440×330)
     — **고장 안 난 것은 안 건드린다.** */
...
test('★★ 칠하는 판에 «정해진 높이»가 있다 — 없으면 사진의 100% 가 조용히 사라진다', () => {
  const r = ruleOf('.edwrap');
  assert.match(r, /(^|;)height:100%/,
    '★★ 칸 높이가 auto 입니다 — 사진의 max-height:100% 가 «없는 것»이 되어\n' +
    '  사진이 칸보다 커지고, 칠한 자리와 지워지는 자리가 1.4배 어긋납니다(2026-08-29 그 사고).');
});

/* ── ③ 사진은 칸을 그대로 채운다 ── */

test('★★ 사진이 칸을 «그대로» 채운다 — 다시 재는 순간 어긋남이 돌아온다', () => {
  const r = ruleOf('.edwrap img');
...
test('★★ 칠하는 판이 사진을 «그대로» 덮는다 — 어긋나면 칠한 곳과 지워지는 곳이 다르다', () => {
  const r = ruleOf('.edwrap canvas');
  assert.match(r, /position:absolute/);
  assert.match(r, /inset:0/, '★★ 사진과 같은 자리에서 시작하지 않습니다');
  assert.match(r, /width:100%;height:100%/, '★★ 사진과 같은 크기가 아닙니다');
  assert.match(r, /touch-action:none/, '★ 폰에서 그으면 화면이 함께 밀립니다');
});
...
function fit() {
  const wrap = { style: {} };
  const ctx = { $: function (id) { return id === 'edWrap' ? wrap : null; }, edRepaint: function () {} };
  vm.createContext(ctx);
  vm.runInContext(cutFn(APP, 'function edFitWrap('), ctx);
  return { fn: ctx.edFitWrap, wrap: wrap };
}
...
test('★ 크기를 모르면 아무것도 안 한다 — 0 을 박으면 칸이 사라진다', () => {
  ...
  /* 칸이 아직 없을 때도 멎지 않는다 */
  const ctx = { $: function () { return null; }, edRepaint: function () {} };
  vm.createContext(ctx);
  vm.runInContext(cutFn(APP, 'function edFitWrap('), ctx);
  ctx.edFitWrap({ naturalWidth: 100, naturalHeight: 50 });
});

test('★★ 사진을 다 불러오면 «실제로» 부른다 — 안 부르면 비가 영영 안 박힌다', () => {
  const fn = cutFn(APP, 'function renderViewerEdit(');
  assert.match(fn, /onload="edFitWrap\(this\)"/,
    '★★ 비를 박는 함수를 만들어 놓고 부르지 않습니다 — 고친 것이 하나도 안 걸립니다');
  /* 비가 바뀌면 칠한 것도 다시 그려야 한다 — 안 그리면 한 박자 어긋난 채로 보인다 */
  assert.match(cutFn(APP, 'function edFitWrap('), /edRepaint\(\)/,
    '★ 비를 바꾸고 칠한 것을 다시 안 그리면 한 박자 어긋나 보입니다');
});

/* ── 어긋나면 어떻게 되는지, 셈으로 한 번 더 ── */

test('★★ 칸과 사진이 다르면 손댄 자리가 어디로 가는지 — 그 셈을 남겨 둔다', () => {
  const wrapH = 744, imgH = 1038, imgTop = -147;   // 칸 위쪽을 0 으로 본 사진의 위쪽
  function drawnAt(clickY) {
    const ratio = (clickY - imgTop) / imgH;        // 손을 재는 쪽의 셈
    return ratio * wrapH;                          // 그려지는 자리
  }
