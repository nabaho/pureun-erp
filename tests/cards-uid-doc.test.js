...
/* ⚠ 2026-08-30 대표 결정 ②㉮ — 열 이름이 「서류이름」에서 「구분」으로 바뀌었다.
     긴 이름을 그대로 적는 대신 「사업자 / 고유」 딱지 하나를 둔다.
     담긴 서류이름은 «안 지운다» — 딱지의 귀띔으로 남는다. */
test('사업자 목록에 「구분」 열이 있다 — 고유번호증을 가려내는 자리다', () => {
  const defs = slice(HTML, 'const COL_DEFS = {', 'function colHidden(');
  assert.match(defs, /\['docName','구분'\]/, '이 열이 없으면 한 장이 어느 쪽인지 모른다');
  /* 가려내는 «판정»이 있어야 한다 — 열 이름만 바뀌고 속이 비면 아무 뜻이 없다 */
  assert.match(HTML, /function bizKind\(/, '★ 사업자·고유를 가리는 판정이 없습니다');
  assert.match(HTML, /isUidDocName\(it && it\.docName\) \? '고유' : '사업자'/,
    '★ 고유를 가려내는 규칙이 바뀌었습니다');
...
test('열 너비가 정해져 있다 — 딱지 하나라 좁아도 된다', () => {
  /* ⚠ 2026-08-30 대표 지시로 「사업자 / 고유」 딱지 하나가 되어 62px 로 줄였다.
     값을 못 박지 않고 «정해져 있는가»와 «예전만큼 넓지 않은가»를 본다. */
  const m = HTML.match(/biz:\s*\{[^}]*docName:'([^']+)'/);
  assert.ok(m, '★ 서류이름 칸 너비가 안 정해져 있습니다 — 칸이 찌그러집니다');
  const px = Number((m[1].match(/^(\d+)px$/) || [])[1]);
  const pct = Number((m[1].match(/^([\d.]+)%$/) || [])[1]);
  assert.ok(px ? px <= 90 : pct <= 8,
    '★ 딱지 하나뿐인데 칸이 넓습니다: ' + m[1]);
});

test('칸별 찾기로 걸린다 — 새 거르개를 만들지 않았다', () => {
  const body = slice(HTML, '/* 칸별 검색 필터 */', '/* 조건 필터: 전화있음');
  /* 깔때기는 「사업자」·「고유」라는 «판정»을 넘긴다 — 값으로만 찾으면 하나도 안 걸린다.
     ⚠ 서류이름 «글자»로 찾던 길(2026-08-26 결정)도 함께 남아 있어야 한다 —
       「고유번호」를 쳐서 거르던 그 길이다. 둘 다 본다. */
  assert.match(body, /bizKind\(it\) === want/, '★ 깔때기가 넘긴 판정으로 못 거릅니다');
  assert.match(body, /!has\('docName', want\)/, '★ 서류이름 글자로 찾던 길이 사라졌습니다');
