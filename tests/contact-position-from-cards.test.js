/* 기업정보함에서 담당자를 가져와도 «직급» 이 빈칸으로 보이던 것 (2026-08-03 고침, 2026-08-15 반영)

   ★ 담당자의 직책이 두 칸에 나뉘어 있다 — role 과 position.
     화면마다 읽는 칸이 다르다: 계약창은 role, 업체관리·컨설팅은 position.
     기업정보함에서 가져오는 자리가 role 한 칸에만 써 넣어, 업체관리에서는 직급이
     내내 빈칸이거나 옛 값 그대로였다.
...
t('어디서 왔는지 남긴다', [c.pcId, c.pcFrom], ['card-1', '기업정보함']);
...
/* 기업정보함에서 온 담당자가 아니면 갱신할 근거가 없다 — 엉뚱한 사람 정보로 덮으면 안 된다 */
t('기업정보함에서 온 담당자만 갱신한다', /if\(!ct\.pcId\)\{ showToast\('기업정보함에서 가져온 담당자가 아닙니다'\); return; \}/.test(REFRESH), true);
t('기업정보함에서 사라졌으면 기존 정보를 지키고 알린다',
