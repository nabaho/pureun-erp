...
test('★★ 「폰 문자함에 하나 문자가 없습니다」는 «끝까지 읽었을 때만» 말한다', () => {
  ...
  /* 단정하는 말은 «끝까지 읽었을 때만» 나가야 한다.
     ⚠ 볼 곳은 «칸의 제목»이다 — 사람이 실제로 읽는 한 줄이 거기다.
       설명 꼬리에만 조건이 달려 있으면 제목은 여전히 「없습니다」라고 단정한다
       (2026-08-30 일부러 깨 보니 여기가 그대로 통과했다).
     ⚠ pairTxt 는 어느 칸이든 제목 끝에 붙는다 — 그 앞까지가 제목이다. */
  const iPair = body.indexOf('pairTxt', iNone);
  assert.ok(iPair > iNone, '칸 제목의 끝을 못 찾았습니다');
  assert.match(body.slice(iNone, iPair), /d\.sweepReadOk === true/,
    '★★ 제목이 읽기 성공을 안 따지면, 못 읽은 것을 「없습니다」라고 단정합니다');
});
