...
test('★ 화면과 서버가 같은 셈으로 시간을 말한다', () => {
  /* 화면이 「22분」이라 해 놓고 실제가 다르면 아무도 못 믿는다.
     ⚠ 2026-09-02 이 검사가 흠을 잡아냈다 — 서버 셈을 고치자 화면이 곧 걸렸다.
       한 바퀴 경계(20·21)와 상한(400)까지 함께 본다. */
  const C = load();
  [1, 10, 20, 21, 88, 300, bulk.MAX_BULK].forEach(n => {
    assert.equal(C.bulkEta(n, 15), bulk.etaText(n, 15000), n + '곳에서 화면과 서버가 다릅니다');
  });
  /* 간격이 발송기보다 느린 쪽도 — 한쪽만 맞고 다른 쪽이 어긋나는 일을 막는다 */
  [5, 20, 88].forEach(n => {
    assert.equal(C.bulkEta(n, 600), bulk.etaText(n, 600000), n + '곳(600초 간격)에서 다릅니다');
  });
});

test('한 시간이 넘으면 시간·분으로 말한다', () => {
  const C = load();
  assert.equal(C.bulkEta(300, 15), '약 3시간 45분');
});
...
test('★ 보낼 곳·빠지는 곳·걸리는 시간을 모두 보여준다', () => {
  ...
  assert.match(txt, /걸리는 시간 약 1시간 15분/);
...
test('★ 왜 오래 걸리는지 «실제로 발목을 잡는 쪽»을 적는다', () => {
  /* 「약 1시간 15분 (15초에 한 통씩)」이라 적으면 88곳×15초=22분이라
     셈이 안 맞아 보인다. 그러면 그 숫자를 못 믿게 된다. */
  const C = load();
  const 빠른간격 = C.bulkSummary({ ready:5 }, 15, []).find(r=>r.k==='걸리는 시간').v;
  assert.match(빠른간격, /발송기가 15분마다 20통씩/, '발목을 잡는 쪽이 발송기인데 간격을 적습니다');

  /* 간격이 발송기보다 느리면 그때는 간격이 까닭이다 */
  const 느린간격 = C.bulkSummary({ ready:5 }, 600, []).find(r=>r.k==='걸리는 시간').v;
  assert.match(느린간격, /600초에 한 통씩/, '이때는 간격이 까닭입니다');
});
...
/* ⚠ 2026-08-30 «묻는 자리»(toggleNoMail)와 «적는 자리»(nmSet)를 갈랐고, 묻는 방법도
     브라우저 confirm 에서 가운데 창(puAsk)으로 바뀌었다(대표 지시 「다음메일처럼 중앙에」).
     지키는 규칙은 그대로다 — 표시를 달고, 저장하고, «묻고» 켠다. 그래서 둘을 함께 본다.
     자세한 것은 tests/cards-nomail-ask.test.js. */
