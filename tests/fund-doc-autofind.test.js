...
test('못 찾았을 때 왜인지 말해 준다', () => {
  const r = grabFn('renderDocFind');
  assert.match(r, /못 찾음/, '');
  assert.match(r, /판독해 두면 다음부터는 여기 저절로 나옵니다/,
    '못 찾은 채로 두면 사람이 기능이 고장 난 줄 안다');
  /* 2026-09-06: 까닭만 말하고 끝나면 «막다른 끝»이다 — 사진첩에 넣어 뒀는데도
     제목에 그 낱말이 없으면 당겨올 길이 사라진다. 거기서 바로 고르게 잇는다. */
  assert.match(r, /docFindBrowse\(/, '못 찾았을 때 직접 고를 길이 없다');
  assert.match(r, /사진첩에서 직접 고르기/, '무엇을 하는 단추인지 안 보인다');
});
