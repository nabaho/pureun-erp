...
test('★★ 바탕(base)은 «그 HTML 이 온 곳»을 가리킨다', () => {
  /* ⚠⚠ 2026-09-03 뒤집혔다. 이 검사는 원래 「바탕은 늘 우리 사본」을 못 박고 있었고,
       그 때문에 «틀린 것을 지키며» 초록이었다 — 미리보기가 민무늬로 나왔는데도.
     ★ 까닭: 「이 쪽 읽어오기」는 «옛 홈페이지»를 읽는데, 그 쪽은 주소를 뿌리부터
       (/files/…) 쓴다. 뿌리부터 쓰는 주소는 base 를 어디로 잡든 «오리진»에 붙으므로,
       base 를 우리 사본으로 잡으면 nabaho.github.io/files/… 를 찾다가 다 404 가 난다.
       (실측 2026-09-03 — 옛 홈페이지: CSS 뿌리부터 1·그림 뿌리부터 7 /
                          우리 사본: CSS 상대 1·그림 상대 11)
     ★ 그래서 규칙은 「늘 사본」이 아니라 «온 곳을 따라간다»이다:
       · 쪽(주요업무 등)  → 옛 홈페이지에서 왔다 → 그 쪽 주소
       · 구성원          → 새 홈페이지에서 왔다(상대 주소) → 우리 사본(같은 곳) */
  const 쪽 = 함수('쪽바탕');
  assert.match(쪽, /PuHomeExport\.ORIGIN/,
    '★★ 쪽 미리보기의 바탕이 «읽어 온 그 홈페이지»가 아니다 — 스타일·그림이 통째로 깨진다');
  assert.match(쪽, /\+ *mid/, '★ 어느 쪽인지 주소에 안 넣는다');

  const 사본 = 함수('사본바탕');
  assert.match(사본, /'site\/'/, '★ 구성원 바탕이 우리 사본이 아니다');
  assert.match(사본, /location\.href/,
    '★ 주소를 박아 두었다 — 이 화면이 어디로 옮겨가도 따라오게 할 것');

  /* 화면이 그 «바탕»을 실제로 넘기는가 — 안 넘기면 위 둘이 있어도 소용없다.
     ⚠ 처음에 «어딘가에 쪽바탕이 있으면 통과»로 썼다가 고쳤다 —
       세 자리 중 한 곳만 사본바탕으로 바꿔도 통과해 버렸다(그게 바로 이번 고장이다).
       「어딘가에 있나」가 아니라 «글자를 돌려주는 곳마다 바탕이 붙었나»를 본다. */
  const m = 함수('미리보기HTML');
  const 돌려주는것 = m.match(/return \{[^}]*\}/g) || [];
  const 글자있는것 = 돌려주는것.filter(function (r) { return /html:/.test(r); });
  assert.ok(글자있는것.length >= 3,
    '★ 글자를 돌려주는 곳을 ' + 글자있는것.length + '군데만 찾았다 — 훑기가 헛돌고 있다');
  const 바탕없는것 = 글자있는것.filter(function (r) { return !/바탕:/.test(r); });
  assert.deepStrictEqual(바탕없는것, [],
    '★★ 바탕 없이 글자만 돌려주는 곳이 있다 — 거기서 «민무늬»로 나온다');

  /* ★ 우리 사본은 «구성원 쪽에만» 맞다 — 쪽은 옛 홈페이지에서 오고 주소를 뿌리부터 쓴다 */
  assert.ok(!/사본바탕\(mid\)/.test(m),
    '★★ 쪽 바탕을 우리 사본으로 잡았다 — 뿌리부터 쓰는 주소가 다 404 가 되어 민무늬가 된다');
  assert.match(m, /사본바탕\('people'\)/, '★ 구성원 바탕이 우리 사본이 아니다');

  assert.match(함수('prevHtml'), /base끼우기\(r\.html, *r\.바탕\)/,
    '★★ 틀에 바탕을 안 넘긴다 — 넘기지 않으면 민무늬로 나온다');
});
