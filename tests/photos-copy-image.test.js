...
test('★ 고른 것이 있을 때 단추가 보인다 — 만들고 안 보여 주면 없는 것과 같다', () => {
  /* ⚠ 2026-09-03 다시 겨눔 — 종전에는 "['dlBtn', 'cpBtn'" 이라는 «글자 그대로»를 찾았다.
     그래서 그 사이에 단추가 하나 늘자(✉️ 메일) 기능은 그대로인데 검사만 울었다.
     지킬 것은 «cpBtn 이 그 목록에 들어 있는가»다. */
  const m = /\[([^\]]*?)\]\.forEach\(function \(id\) \{\s*\$\(id\)\.style\.display/.exec(app);
  assert.ok(m, '★ 고른 뒤 단추를 보이게 하는 목록을 못 찾았습니다');
  assert.ok(m[1].indexOf("'cpBtn'") >= 0, '★ cpBtn 을 보이게 하는 목록에 안 넣었습니다');
});
