...
test('★ 콘솔과 «한 곳도» 다르지 않다', () => {
  ...
  /* ★ 2026-09-03 — 경력관리 「등록 신청함」(kcareer_inbox)이 생겼다. 만들개에는 들어갔고
     **콘솔에는 아직 안 올라갔다.** 그래서 여기 적어 둔다 — 대표가 게시하시면 콘솔 원문
     파일을 그 내용으로 맞추고 이 줄을 지운다.
     ⚠ 안 올라간 동안에는 **직원이 등록 신청을 올려도 서버가 막는다**(규칙이 없으니
       이름 없는 자리로 떨어진다). 화면은 되는 것처럼 보이므로 조용한 실패다. */
  const PENDING = ['/kcareer_inbox'];
