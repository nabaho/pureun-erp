...
test('★ 콘솔과 다른 곳은 «일부러 고친 세 곳»뿐이다', () => {
  ...
  /* ⏳ 2026-08-30 — 콘솔에 «아직 안 올라간» 자리 하나:
       /data/staff_colors — 직원 색표(사번 → 색). 푸른이알피 법인대시보드가 정하고
       컨설팅일정이 읽는다(대표 지시 「전체 시스템 색을 일치시켜라」).
       읽기는 직원 누구나, 쓰기는 관리자·위임관리인.
     ⚠ 이 줄은 «임시»다. 대표가 콘솔에 게시하시면 콘솔 원문 파일을 그 내용으로
       갈아 끼우고 이 목록을 다시 «비운다». 비우지 않고 두면, 다음에 규칙이
       어긋났을 때 이 검사가 그것을 못 알려 준다. */
  /* ⏳ 2026-08-30 — 하나 더:
       /data/cons_type_colors — 사업(컨설팅 종류) 색표(푸른이알피 유형 코드 → 색).
       «컨설팅일정»이 정하고 다른 앱이 읽는다(대표 결정 「㉮」). 사람 색과 방향이
       반대인 까닭은 푸른이알피의 컨설팅 유형 자료에 색 칸이 «아예 없어서»다.
  ...
  /* ⏳ 2026-09-02 — 경력관리 직원 공개용 사본:
       /kcareer_pub — 대표가 «경력관리 세 통만»(위촉장·자격·학력) 골라 올린 사본.
       읽기는 재직 직원 전원, 쓰기는 관리자(대표)만.
       대표 지시 「경력관리 이부분만 다른 직원들이 볼 수 있게」 → 방식 「나」 승인.
       ⚠ 대표 칸(kcareer/{uid})은 «안 열었다» — 그 칸에는 실적·비용·개인정보·신분증이
         함께 들어 있어 통째로 열면 경력관리만 보여 줄 수가 없다.
     ⚠ 대표가 콘솔에 게시하시면 콘솔 원문 파일을 그 내용으로 갈아 끼우고 이 줄을 «지운다».
       안 지우면 다음에 규칙이 어긋났을 때 이 검사가 그것을 못 알려 준다. */
  /* ⏳ 2026-09-02 — 뉴스레터:
       /newsletter — 주간뉴스레터의 설정·회차 초안·받는 명단.
       읽기·쓰기 모두 **총괄관리자만**(홈페이지 관리와 같은 잣대).
       ⚠ 지금 콘솔에서는 이름 없는 자리($other)로 떨어져 **전 직원이 읽고 쓴다** —
         거래처 담당자 이름·이메일이 든 명단이고, 법인 이름으로 나갈 편지의 초안이다.
         게시하면 좁혀진다.
       대표 지시 「푸른내외관리에 뉴스레터관리를 넣고 매주 관리하고 싶다」. */
  const PENDING = ['/data/staff_colors', '/data/cons_type_colors', '/data/ledger_batches',
    '/puphotos/access_log', '/kcareer_pub', '/newsletter'];
