...
test('★ 콘솔과 «한 곳도» 다르지 않다', () => {
  ...
  /* ★ 2026-09-07 — 취업규칙 서고(사례집) 한 칸이 «기다리는 중»이다.
       /rules_mgmt/casebook  네 층(index·rev·text·idx).
                             읽기는 재직 직원 전체 — 보관함(내 것 + 남의 완료본)보다
                             «넓다». 사례집이 목적이라 설계서 §6 이 그렇게 정했고,
                             그래서 올리는 화면에 「직원 전체가 볼 수 있습니다」를 적었다.
                             쓰기는 rev·text 가 올린 사람 또는 관리자(orig 와 같은 모양),
                             index·idx 는 재직 직원(같은 사업장에 다른 담당자가 회차를
                             더할 수 있어야 한다 — 이유는 만들개 주석에 적어 두었다).
     ⚠ 이 자리가 콘솔에 올라가기 전까지 서고에 아무것도 «안 담긴다»(permission_denied).
       올라가면 이 항목을 지우고 콘솔 원문 파일을 그 판으로 갈아 끼운다. */
  const PENDING = ['/rules_mgmt/casebook'];
