...
// (2026-08-09) 단추 글자는 상황에 따라 바뀐다 — 상담접수와 이름이 맞으면 「상담료」
// 처리 칸 단추는 크기를 맞추려고 글자를 짧게 뒀다 (무엇인지는 마우스를 올리면 나온다)
ok('후보가 없을 때 버튼이 나온다', /_cm\?'상담료':'등록'/.test(src));
ok('상담접수와 이름이 맞으면 그 건에 이어 준다', /consultId:\(_cm&&_cm\.id\)\|\|''/.test(src));
