...
/* ⚠ 여기가 **지금 아는 새는 길**이다.
   · startRead — 사진을 **올리는 중** 대기열에서 자동으로 판독한다. 가림 화면을
     띄울 수가 없다(서른 장을 줄 세워 놓고 한 장씩 물을 수는 없다).
   · readPhoto — 격자에서 부를 때는 가린 사본(masked)을 받아 그것만 읽지만,
     **대기열이 부를 때는 masked 없이** 불린다(pumpRead).
   고치는 길은 셋뿐이고 어느 쪽이든 대표 결정이 필요하다:
     ① 서류 자동 판독을 끈다 ② 이대로 감수한다 ③ 기계가 찾아 자동으로 가리고 보낸다
        — ③은 「사람이 한 번 본다」를 스스로 무너뜨리는 것이라 그냥 못 고른다. */
