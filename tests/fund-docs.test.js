 ...
 * 그래서 감사보고서는 감사가 «각자» 한 장씩 쓴다(원본: T공동 2019년 감사보고서, 표 없는 한 장짜리 서한).
...
test('이사장을 한쪽 위원으로 밀어 넣지 않는다', () => {
  /* 협의회는 노·사 동수다. 명부가 측을 말해 주지 않는 이사장을 사용자위원에 넣으면
     구성이 틀린 회의록이 된다 — 참석위원과 서명란(위원장)에만 넣는다. */
  const m = docBody('ops_minutes_close', FUND, []);
  const row = (m.match(/사용자위원<\/th><td>([^<]*)</) || [])[1] || '';
  assert.ok(!/갑 대표/.test(row), '이사장이 사용자위원 칸에 들어갔다: ' + row);
  assert.match(m, /참석위원[\s\S]{0,120}갑 대표/, '참석위원에는 들어가야 한다');
  assert.match(m, /위원장\(이사장\): 갑 대표/, '서명란에 위원장으로 적어야 한다');
});
