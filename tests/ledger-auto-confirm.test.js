...
/* 이게 빠지면 확정분이 「성과 미반영」에 쌓인다 — 연쇄 처리의 핵심.
   ※ 글자수(«700자 안에») 로 재지 않는다 — 주석 한 줄만 늘어도 엉뚱하게 깨진다
     (2026-08-16 에 실제로 그렇게 깨졌다). 블록을 이름으로 잘라 그 «안에» 있는지 본다. */
const _aS = src.indexOf('if(opts.withPerf && !isAdv){');
const _aE = src.indexOf('// 개인수익·원천징수분은', _aS);
const AUTO = (_aS >= 0 && _aE > _aS) ? src.slice(_aS, _aE) : '';
ok('자동확정 블록을 잘라냈다', AUTO.length > 0);
ok('saveIncome 이 opts.withPerf 일 때 성과를 나눈다', /calcPerfShares\(/.test(AUTO));
/* 창을 거친 건과 «같은 기본 분할» 을 쓴다 (2026-08-16 대표 지시).
   여기가 빠지면 통장에서 자동 확정한 건만 주담당 100% 로 남아,
   주담당이 요율 0%(대표님) 면 성과급이 통째로 사라진다. */
ok('창과 같은 기본 분할 함수를 쓴다', /erpDefaultSplit\(/.test(AUTO));
