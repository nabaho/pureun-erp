...
t('월별 손익 표 상자가 overflow:auto + maxHeight',
  /viewMode==='month' \? h\('div',\{style:\{border:'1px solid #e2e8f0',borderRadius:'8px',overflow:'auto',maxHeight:'calc\(100vh - 260px\)'\}\}/.test(F), true);
t('월별 손익 머리글이 sticky', /viewMode==='month'[\s\S]{0,400}?h\('thead',\{style:\{position:'sticky',top:0,zIndex:2\}\}/.test(F), true);
t('카테고리별 지출 표 상자도 overflow:auto + maxHeight',
  /viewMode==='category' \? h\('div',\{style:\{border:'1px solid #e2e8f0',borderRadius:'8px',overflow:'auto',maxHeight:'calc\(100vh - 260px\)'\}\}/.test(F), true);
