...
/* ── ② 카드 칸으로 옮기기 ── */
test('★★ 카드 줄의 꼬리를 적요 옆에 «안 붙인다»', () => {
  const src = bare(ERP);
  /* ⚠ 2026-08-30: 둘째 «줄»이 아니라 같은 줄의 «옆»이 되었다
     (대표: 「한줄로 정리해라… 2줄로 절대 만들지마라」).
     못 박을 것은 모양이 아니라 «카드 줄에서는 안 그린다» 는 규칙이다. */
  assert.ok(/\(row\.src!=='card'\) && erpNoteWorth\(row\.note\)\s*&& h\('span'/.test(src),
    '★ 적요 옆에 「하나카드 승인 문자 · ****9950」이 그대로 남는다 — 가게 이름을 가린다');
});
