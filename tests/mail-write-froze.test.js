...
test('★★ 도구줄·본문은 덩이 «밖»에 있다 — 안에 넣으면 화면 절반이 머리가 된다', () => {
  const h = load().mailWriteHtml();
  const open = h.indexOf('class="cphead"');
  const bar  = h.indexOf('class="edbar"');
  const body = h.indexOf('id="cpBody"');
  assert.ok(open > 0 && bar > open && body > bar, '차례가 어긋났습니다');
  /* 덩이가 도구줄 앞에서 «닫혔는지» — 여닫이 수로 센다 */
  const seg = h.slice(h.lastIndexOf('<div', open), h.lastIndexOf('<div', bar));
