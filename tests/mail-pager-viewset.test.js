...
/* 목록 아래 영역 — 쪽번호 · 보기설정 (대표 지시 2026-08-30, 다음메일 캡처3·4)
   ...
   ★ 여기서 못 박는 것
     ...
     ⑨ ★ 새로 지은 이름이 «이미 있는 이름»과 안 겹친다
        (실제로 mbGo 가 겹쳐 쪽 넘김이 통째로 안 먹었다 — 화면에서 잡았다)
...
/* ══════ ⑨ 이름이 안 겹친다 ══════ */
test('★★ 새로 지은 이름이 «이미 있는 이름»과 안 겹친다', () => {
  /* ⚠ 이 검사가 있는 까닭 — mbGo 로 지었더니 「칸 열기」 mbGo 가 이미 있었다.
       뒤에 선언된 것이 이기므로 쪽 넘김이 통째로 안 먹었는데, 구문오류도 없고
       검사도 다 통과했다. 화면에서 눌러 보고서야 알았다. */
  /* 예전에 겹쳐 있던 toggleNoMail 은 2026-08-30 에 하나로 합쳤다(묻는 쪽을 남겼다).
     여기는 이제 비어 있어야 한다 — 새로 겹치면 그 자리에서 걸린다. */
  const KNOWN = [];
  const names = {};
