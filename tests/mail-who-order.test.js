/* 푸른 메일 — 담당자 목록 (대표 지시 2026-08-29)
   ...
   ⑤ 아래 세 줄(담당 모름·자문종료·이메일 잇기)도 «같은 자리»에 선다 */
...
test('★ 끌어 옮긴 차례가 있어도 「나」는 맨 위 그대로', () => {
  ...
  const c = load({ order: { '@권형하': 99, '@나래': 0 } });
...
test('★★ 끌어 옮긴 차례가 사번순보다 «이긴다»', () => {
  const c = load({ order: { '@나래': 0, '@하윤서': 1, '@가온': 2 } });
...
test('★ 끌어 놓으면 차례가 «남는다» — 다음에 열어도 그대로', () => {
  const c = load();
  c.state.mbDrag = { kind:'bin', id:'@나래' };
  c.mbDrop({ preventDefault(){} }, '@하윤서', null);
  const saved = c._held.wrote['pucards/config/mailOrder'];
  assert.ok(saved, '차례를 저장하지 않았습니다');
  assert.ok(Number(saved['@나래']) < Number(saved['@가온']),
    '옮긴 자리가 안 담겼습니다: ' + JSON.stringify(saved));
});

test('★★ 업무 칸 차례를 옮겨도 담당자 차례가 «안 지워진다»', () => {
  /* 둘이 같은 자리(config/mailOrder)를 쓴다. 통째로 덮어쓰면 한쪽이 사라진다. */
  /* ⚠ 갈래를 두지 않는다 — 「칸이 하나뿐이면 그냥 통과」로 두었더니 이 검사가
     헛돌아, 차례를 통째로 덮어써도 걸리지 않았다(2026-08-29 뮤테이션에서 잡음). */
  const c = load({ order: { '@나래': 0, '@하윤서': 1, '@가온': 2 } });
  const bins = c.mbBins();
  ...
  const saved = c._held.wrote['pucards/config/mailOrder'] || c.__order();
  ['@나래','@하윤서','@가온'].forEach(k =>
    assert.ok(k in saved, '업무 칸을 옮겼더니 담당자 차례(' + k + ')가 지워졌습니다: '
      + JSON.stringify(saved)));
  assert.ok(bins[0].id in saved, '업무 칸 차례가 안 담겼습니다');
});

test('서로 다른 목록끼리는 «안 섞인다» — 담당자를 업무 칸 자리로 끌어도', () => {
  const c = load({ order: { '@나래': 0 } });
  const before = JSON.stringify(c.__order());
  c.state.mbDrag = { kind:'bin', id:'@나래' };
  c.mbDrop({ preventDefault(){} }, 'B1', null);
  assert.equal(JSON.stringify(c.__order()), before, '담당자를 업무 칸 사이에 끼웠습니다');
});
...
/* ══════ 옆줄의 «모든 줄»이 같은 칸 차례를 갖는다 ══════
   ...
   ⚠ 개발 서버 자리가 다른 세션으로 차 있어 이번에는 브라우저로 못 쟀다. 이 검사가 그 자리를
     메우고, 브라우저가 있을 때도 «되돌림을 막는 그물»로 남는다. */
test('★★ 옆줄의 모든 줄이 «같은 칸 차례»다 — 하나라도 다르면 그 줄에서 이름이 밀린다', () => {
  /* ⚠ «두 대시보드를 다» 본다. 담당자 쪽만 보다가 업무 칸이 「점 → 손잡이」로
     거꾸로인 것을 놓쳤다(2026-08-29). 이름 x 는 같아 재면 0px 이지만, 손을 얹으면
     손잡이가 «다른 자리»에서 나타나 눈에는 계속 어긋나 보인다. */
  const c = load();
  c.state.mbDash = 'who';   const hw = c.mailSideHtml();
  c.mbMemoClear();
  c.state.mbDash = 'topic'; const ht = c.mailSideHtml();
  const h = hw + ht;
  const rows = h.match(/<div class="dm-f sub[^"]*"[\s\S]*?<\/div>/g) || [];
...
test('★ 숫자는 어느 줄에도 «하나»뿐이다 — 둘이면 그 줄만 오른쪽이 어긋난다', () => {
  ...
  c.mbMemoClear();
  c.state.mbDash = 'topic'; const ht = c.mailSideHtml();
  const rows = (hw + ht).match(/<div class="dm-f sub[^"]*"[\s\S]*?<\/div>/g) || [];
