...
/* 검사고정-허용 16.7: 대표 지시 「1CM 줄여달라」를 이 화면에 옮긴 값이다 —
   표 폭 1620px 에서 1cm = 37.8px = 2.33% 이므로 19 − 2.33 ≒ 16.7.
   ⚠ calc(19% - 1cm) 로 두었다가 되돌렸다. <col> 에서는 엉뚱하게 풀려 270px 이어야 할
     칸이 375px 로 «오히려 넓어졌다»(실측). 되살리지 말 것. */
test('★ 상호가 1cm 만큼 좁아졌다 — 19% 로 되돌아가지 않았다', () => {
  ...
  assert.ok(pct >= 16.4 && pct <= 17.0,
    '★ 상호 폭이 ' + m[1] + ' 다 — 「1cm 줄이기」는 19% → 16.7% 다');
});
