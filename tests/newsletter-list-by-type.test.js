...
test('★★ 첫 값은 «자문»이다 (대표 지시 2026-09-03 「자문 기본값으로」)', () => {
  /* 검사고정-허용: 이 값은 «취향이 아니라 정한 것»이다.
     유형별로 갈라 보니 활성 206곳 가운데 자문 84 · 급여 114 였고,
     자문 계약 없이 급여 대행만 하는 곳까지 뉴스레터를 받고 있었다.
     법으로는 거래관계 예외 안이라 문제가 아니었지만, 대표께서 자문에만 보내기로 정하셨다.
     ⚠ 첫 값을 «전체»로 되돌리면 정하신 것과 달리 급여·노조·기금까지 나간다 —
       그것이 이 검사가 막는 일이다. */
  const m = /var App = \{([\s\S]*?)\n\};/.exec(news);
  assert.ok(m, 'App 상태를 못 찾았다');
  assert.ok(/유형:\s*'자문'/.test(m[1]),
    '★ 첫 값이 「자문」이 아니다 — 대표께서 정하신 것과 달리 급여·노조·기금까지 나간다');
});
...
test('★ 경고(⚠)는 «넓힌 쪽»에 붙는다 — 기본이 자문이므로', () => {
  /* 자문만 나가는 것은 정한 대로다. 알아야 할 일은 «전체로 넓혔을 때»다.
     경고를 자문 쪽에 붙이면 늘 떠 있게 되고, 늘 떠 있는 경고는 곧 아무도 안 본다. */
  const m = /async function 진짜보내기\(\)\{([\s\S]*?)\n\}/.exec(news);
  const 줄 = m[1].split('\n');
  const 한유형 = 줄.find(l => /에만 나갑니다/.test(l)) || '';
  const 전체 = 줄.find(l => /모두 나갑니다/.test(l)) || '';
  assert.ok(!/⚠/.test(한유형), '자문만 나갈 때 경고가 붙어 있다 — 늘 뜨는 경고는 안 읽힌다');
