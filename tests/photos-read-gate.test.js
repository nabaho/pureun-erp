...
test('★★ 「그냥 사진」 답은 «사람이 그랬다»고 적는다 — AI 판정으로 꾸미지 않는다', () => {
  ...
  assert.match(fn, /confirm\(/,
    '★★ 되돌릴 수 있어도 한 번은 물어야 합니다 — 서류가 섞여 있으면 그 서류가\n' +
    '  스스로 안 읽히고, 모르고 지나가면 한 달 뒤에야 압니다.');
});
...
test('★★ 한도에 걸리면 자동 줄을 «비운다» — 남은 것이 한 장씩 두드리지 않게', () => {
  ...
  assert.match(fn, /readQ\.splice\(0\)/, '★ 줄을 비우지 않으면 한 장씩 차례로 다 두드립니다');
  assert.match(fn, /_queuedRead = false/,
    '★★ 표를 안 내리면 한도가 풀린 뒤에도 그 사진들은 「이미 줄에 있다」로 남아\n' +
    '  **영영 다시 못 걸립니다.**');
});
