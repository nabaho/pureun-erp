...
test('★ 명부 딱지는 홈페이지를 «한 번도 안 읽어도» 붙는다', () => {
  ...
  /* ⚠ 2026-09-02 대표 지적으로 뒤집혔다: 「불필요한 내용을 줄이고 2줄을 1줄로」.
     날짜를 딱지에 적으면 같은 줄의 설명(detail = 「퇴사일 2026-06-30」)과 겹쳐
     한 줄에 날짜가 «두 번» 적힌다. 그래서 딱지에서는 뺀다 —
     다만 날짜가 어디에도 없어지면 안 된다. 그것을 여기서 함께 지킨다. */
  assert.ok(m.label.indexOf('2026-06-30') < 0,
    '★ 딱지에 날짜를 다시 넣었다 — 설명(detail)에 이미 있어 한 줄에 두 번 적힌다');
  assert.match(m.detail, /2026-06-30/, '★ 퇴사일이 설명(detail)에서도 사라졌다');
  assert.equal(m.leftAt, '2026-06-30', '★ 퇴사일이 자료(leftAt)에서도 사라졌다');
});
