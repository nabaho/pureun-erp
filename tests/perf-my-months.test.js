...
test('★ 까닭 두 가지를 다 적는다 (하나만 적으면 절반은 거짓말)', () => {
  ...
  /* ⚠ 글월 그대로가 아니라 «까닭 둘이 다 있는가» 를 본다 — 2026-08-20 에 석 줄을
     한 줄로 묶으면서 말투가 바뀌었고, 글자로 박아 둔 탓에 멀쩡한 정리가 걸렸다. */
  const n = grab('pcNoneHTML');
  assert.match(n, /붙은 건이 없었거나/, '① 그 달에 붙은 건이 없었다');
  assert.match(n, /내보내(지 않았|지 못했)|안 내보냈/, '② 대표가 아직 발행하지 않았다');
});
...
test('달마다 카드가 따로 나오는 방식은 그대로', () => {
  ...
  /* ⚠ 「한 줄 그대로」가 아니라 «달마다 카드를 만드는가» 를 본다 — 첫 달에만 안내를
     붙이려고 map 을 함수로 감쌌더니 글자로 박아 둔 검사가 걸렸다(2026-08-20). */
  const r = grab('renderPerf');
  assert.match(r, /list\.map\(/, '달 목록을 돌지 않습니다');
  assert.match(r, /pcMonthHTML\(/, '달마다 카드를 만들지 않습니다');
