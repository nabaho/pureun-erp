'use strict';
/* 다른 직원이 찍은 서류를 판독해 기업정보함에 넣기 — 실행: node --test tests/*.test.js
...
test('모아 보내기는 AI 를 다시 부르지 않는다 — 이미 읽은 값을 쓴다', () => {
  ...
  assert.match(fn, /sendCards\(/, '기업정보함으로 보내지 않습니다');
});
...
test('기업정보함에는 **찍은 사람** 이름이 남는다 — 대신 보낸 사람이 아니다', () => {
  /* 대표가 남의 사진을 대신 보낼 때 자기 이름을 적으면 기업정보함의
     「누가 가져온 명함인가」가 틀어진다. */
  const i = app.indexOf('\nfunction sendCards(');
  const j = app.indexOf('\n}', i);
