...
test('서류와 사진을 갈라 그린다', async (t) => {
  ...
  await t.test('일반 사진은 그림이 꽉 찬다 — 카드로 만들지 않는다', () => {
    ...
    /* ⚠ 갈림길의 «조건»이 2026-08-31 에 늘었다 — 보여 줄 글자가 하나도 없는 서류는
       종이 카드로 안 가고 사진을 꽉 채운다(대표 보고 「사진이 모두 안 나온다」:
       판독 전 서류는 제목도 업체도 없어 26px 띠만 남은 **하얀 칸**이었다).
       여기서 지키는 것은 「사진은 카드로 안 만든다」이지 조건이 무엇인가가 아니다. */
    /* ⚠ 2026-09-03 — 조건이 다시 좁아지면서 그 안에 «괄호»가 들어왔다(docTitle(it)).
       [^)]* 는 첫 괄호에서 끊겨 갈림길을 통째로 못 찾는다 — 기능은 멀쩡한데 검사만
       울었다. 여기서 지키는 것은 여전히 「사진은 카드로 안 만든다」 하나다. */
    const mm = /if \(it\.meta\.kind === 'doc' && .+?\) \{/.exec(app);
