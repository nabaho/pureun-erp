...
test('★ 설정이 없으면 부르기 전에 말해 준다', () => {
  ...
  // ★ «어디에» 넣는지까지 맞아야 한다. 그 칸은 포털의 ⚙ 설정에만 있다 —
  //   푸른이알피 환경설정을 가리키면 찾다가 못 찾고 포기한다.
  assert.match(SUM, /시작 화면\(포털\) 왼쪽 아래 \[⚙ 설정\] → 「AI 프록시 주소」/);
  assert.ok(!/환경설정에서 AI 프록시/.test(SUM), '푸른이알피 환경설정에는 그 칸이 없다');
