...
test('★★ 두 대시보드의 «최종 색·크기»가 모두 같다 (계단을 풀어서 견준다)', () => {
  ...
  CASES.forEach(([what, a, b, prop, mayBeNone]) => {
    ...
    /* ⚠ 둘 다 «없음»이면 같다고 볼 수 없다 — 규칙을 통째로 지워도 초록이 된다.
       .ic 로 이름을 합칠 때 실제로 이 구멍이 열렸었다(2026-08-29). */
    if (!x && !y) {
...
test('★ 이 검사가 «못 읽고 지나친» 규칙이 없다 — 있으면 조용히 거짓 초록이 된다', () => {
  ...
  /* 다룰 수 없는 모양이 나오면 «그 이름»을 적어 둔다 — 늘어나면 검사를 손봐야 한다 */
  assert.ok(uniq.length < 40,
    '못 읽는 선택자가 ' + uniq.length + '개로 늘었습니다 — 계단 풀기를 손봐야 합니다:\n  '
