...
test('★ 아이콘 칸이 «이름 하나»다 — 둘이면 언젠가 한쪽만 바뀐다', () => {
  /* ⚠ 예전에는 .ic(담당자)와 .dot(업무 칸) 두 이름이 같은 것을 가리켰다.
     한 규칙으로 묶어 두었는데도, 마크업에서 «차례»가 갈려 업무 칸만
     「점 → 손잡이」로 거꾸로였다(2026-08-29). 이름부터 하나로 합쳤다. */
  assert.ok(src.indexOf('class="dot"') < 0, '아이콘 칸 이름이 아직 둘입니다(.dot 이 남았습니다)');
  assert.match(css, /\.dm-f \.ic\{[^}]*color:/, '아이콘 색을 정한 곳이 없습니다');
  assert.match(css, /\.dm-f\.on \.ic\{[^}]*color:/, '고른 줄 아이콘 색을 정한 곳이 없습니다');
});
