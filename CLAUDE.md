...
## 파이어베이스 보안규칙 — **물어보고, 승인하시면 내가 올린다** (대표 지시 2026-09-03)

「파이어베이스에 매번 내가 이렇게 반복하는것 너무 귀찮은데, 니가 연결하고
내가 확인해서 승인하는 방식을 만들 수 없나?」
「앞으로 여기서 만드는 푸른통합시스템에서는 콘솔 자동등록여부 질문하고
답하면 니가 알아서 수정해라」

**이제 사람이 붙여넣지 않는다.** 어느 연장이든(경력관리·명함첩·급여·사진첩 …)
규칙을 새로 더해야 하는 기능을 만들었으면 이 세 걸음을 밟는다.

```
① 만들개에 규칙을 넣는다   scripts/make-firebase-rules.js   ← 여기만 고친다
② 보여 주고 물어본다       node scripts/rules-deploy.js     ← 아무것도 안 바꾼다
③ 「올려라」 하시면 올린다   node scripts/rules-deploy.js --deploy
```

- ⚠ **물어보는 것을 건너뛰지 말 것.** 규칙 배포는 한 번에 전부를 바꾸고 되돌리기가
  번거로워, 대표 승인 없이 `--deploy` 를 돌리지 않는다.
  다만 «보여 주는 단계»까지는 말없이 해 둔다 — 「규칙이 필요합니다」만 알리고
  사람이 붙여넣게 하는 일을 없애자는 것이 이 지시의 뜻이다.
- ⚠ **사라지는 규칙이 하나라도 있으면 스크립트가 스스로 멈춘다**(종료코드 2).
...
- ⚠ **콘솔에서 손으로 규칙을 고쳤다면 그 전문을 새 기준 파일로 살려 둘 것.**
  안 그러면 다음 배포가 그 손질을 덮으려 하고, 그때 멈춰서 진행이 막힌다.
- 올린 뒤엔 올린 내용이 새 기준으로 자동 저장된다 — 다음에 견줄 자리다.
- 생기는 파일: `docs/rules-paste.json` — 자동 배포가 이것을 올린다.
  사람이 붙여넣을 일이 생길 때만(CLI 가 죽거나 로그인이 풀릴 때) 여는 것이다.
- 읽을 문서: `docs/firebase-rules-적용안내.md` · `docs/firebase-rules-읽어보세요.md`.

**예전 방식을 되살리지 말 것** — 채팅에 규칙 전문을 통째 부어 대표가 붙여넣게
하는 것은 2026-09-03 부터 하지 않는다. 자동 배포가 막힐 때만 그 길을 쓴다.
