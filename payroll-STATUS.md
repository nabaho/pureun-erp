...
## 7. 알려진 이슈
- **화담원 "노임대장"=일용노무비 날짜격자형**(성명·주민·일당·출역 1~31일칸, 급여=일당×출역일수, 기본급/과세 컬럼 없음). 표준 파서로 읽으면 기본급"-"·깨진행 → **급여처리 파일럿서 제외함**(build_pilot). **일용 day-grid 전용 파서(일당×출역=지급액·출역일수) 필요=다음 작업**. 2줄/인(성명줄+연락처/계좌줄)·#VALUE!·병합셀 주의.
