...
5. ~~종료·완료된 건만 자동으로 들어온다~~ → **2026-08-01 실사용 후 변경: 진행 중인 건도 가져온다.**
   사건 13건 중 11건이 `진행중`이어서 "종료만" 규칙으로는 실적이 거의 안 들어왔다.
   상태(`완료`/`진행`)를 달아 함께 가져오고, pu-erp에서 종료되면 **다음 동기화 때 상태·연도만 맞춘다**(§6-2).
   증명서 발급은 `완료` 건만 고르게 한다.
6. 내·외부 구분에서 **수행기관에 '직접'이 들어가면 내부**로 본다(`의뢰기관 직접` = 푸른 자체 실적).
7. ⚠ pu-erp는 `data/{키} = {v:값, u:시각}` 봉투로 저장한다. **모든 읽기는 봉투를 벗겨야 한다**
   (안 벗겨서 컬렉션당 `v`·`u` 2개가 레코드로 세어져 "진행중 8건, 새로 0건"이 나왔다).
...
- **추가 + 상태 맞추기(아래)만 한다.** 그 밖의 필드는 덮어쓰지 않는다(수동 수정 보존).

**상태 맞추기 (2026-08-01 추가)** — 진행 중으로 가져온 건이 pu-erp에서 종료되면
`status`와 비어 있던 `year`만 갱신한다. `buildStatusUpdates(collData, existingRecords)`가
바뀐 것만 골라내며, 기관·고객사·내용처럼 사람이 고칠 수 있는 필드는 건드리지 않는다.
`puRef`가 없는 수동 등록 레코드와 pu-erp에서 사라진 건은 대상이 아니다.
미리보기에 "↻ 이미 있는 실적 N건이 완료로 바뀝니다"로 표시한다.

- 레코드에서 "pu-erp 원본 보기" = puRef로 `data/{coll}/v/{key}`(봉투 안)를 그 자리에서 읽어
