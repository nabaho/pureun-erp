...
(function (global) {
  ...
  global.PuHomeDiff = {
    memberStatus: memberStatus, pageStatus: pageStatus,
...
test('홈페이지를 못 읽었을 때와 빈 결과를 구분한다', () => {
  assert.match(html, /length === 0|length===0/,
    '읽어낸 사람이 0명일 때 대조를 반영하지 않는 가드가 있어야 한다');
});
```
...
async function checkHomepage() {
  ...
  if (live.length === 0) {                 // 구조가 바뀐 것이지 사람이 사라진 게 아니다.
