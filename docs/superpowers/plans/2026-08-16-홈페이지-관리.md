...
- Produces:
  ...
  - `PuHomeExport.riskyLines(careers)` → 꺾쇠가 든 줄 목록(경고용). 지우지 않는다
  - `PuHomeExport.editUrl(kind, key)` → 편집 화면 주소. `kind` 는 `'member'` 또는 `'page'`. 글 번호가 비었거나 모르는 kind 면 `null`
...
/* ★ 2026-08-16 대표 결정: 꺾쇠는 지우지 않고 경고한다.
   지우는 규칙을 세 번 고쳤는데 세 번 다 사람이 쓴 표기(<S> 등급, <PM>, <노무담당>)를
   말없이 삭제했다. 찌꺼기와 사람 글자를 기계가 가릴 방법이 없다.
   글자가 사라지는 쪽보다 남겨두고 알리는 쪽이 낫다. */
...
(function (global) {
  ...
  function riskyLines(careers) {
    const list = Array.isArray(careers) ? careers : [careers];
    return list.map(clean).filter(function (l) { return l && /[<>]/.test(l); });
  }
