...
/* ── ④-1 상세는 «라벨»로 읽는다 (2026-09-04 실제 화면으로 알아냈다) ────────
   엿보기로 로그인한 상세를 처음 보고 알아낸 것 —
   글자로 뽑으면 이런 차례로 온다:
     … 자료번호 3430 제목 2026년 7월 사업체노동력조사 … 설명 … 출처(소관) 재정경제부
       작성일 2026-08-27 첨부파일1 260827_보도자료_….pdf 이전 페이지는 …

   ⚠ 예전에는 <title> 을 제목으로 삼아 «ilabor 3.0» 이 들어왔고, 본문에는
     차림표·바닥글이 통째로 섞였다(고용노동부 공인노무사법 지원금 코로나 …).
     라벨 사이만 잘라내면 그 둘이 한꺼번에 풀린다.
   ⚠ 라벨 이름을 못 박는 것이 아니라 «라벨과 라벨 사이»를 잘라낸다 —
     한 라벨이 없어도 나머지는 그대로 읽힌다.
   ⚠ 「이전 페이지는」 부터가 꼬리다. 거기서 끊지 않으면 바닥글의 사업자등록번호까지
     설명에 들어온다. */
var 라벨들 = ['자료번호', '제목', '설명', '출처(소관)', '출처', '작성일', '첨부파일'];

function 라벨로읽기(글) {
  var s = String(글 == null ? '' : 글);
  /* 꼬리를 먼저 자른다 */
  var 꼬리 = s.search(/이전\s*페이지는|이용약관|COPYRIGHT/i);
  if (꼬리 > 0) s = s.slice(0, 꼬리);
  var 시작 = s.indexOf('자료번호');
  if (시작 < 0) return null;                 /* 라벨이 없다 — 옛 방식으로 되돌아간다 */
  s = s.slice(시작);

  /* 라벨이 나오는 자리를 다 찾아 «사이»를 값으로 삼는다 */
  var 자리 = [];
  라벨들.forEach(function (L) {
    var re = new RegExp(L.replace(/[()]/g, '\\$&') + '\\d*', 'g');
    var m;
    while ((m = re.exec(s))) 자리.push({ 이름: L, 시작: m.index, 끝: m.index + m[0].length });
  });
  자리.sort(function (a, b) { return a.시작 - b.시작; });
  /* 겹치는 것 걷기 — 「출처(소관)」과 「출처」가 같은 자리에서 둘 다 잡힌다 */
  var 골라낸 = [];
  자리.forEach(function (x) {
    var 앞 = 골라낸[골라낸.length - 1];
    if (앞 && x.시작 < 앞.끝) { if (x.끝 > 앞.끝) 앞.끝 = x.끝; return; }
    골라낸.push(x);
  });

  var 안 = {};
  골라낸.forEach(function (x, i) {
    var 다음 = 골라낸[i + 1];
    var 값 = s.slice(x.끝, 다음 ? 다음.시작 : s.length).replace(/\s+/g, ' ').trim();
    var 키 = (x.이름 === '출처(소관)') ? '출처' : x.이름;
    if (키 === '첨부파일') { (안.첨부이름 = 안.첨부이름 || []).push(값); return; }
    if (!안[키]) 안[키] = 값;
  });
  return 안;
}
...
function 상세읽기(html) {
  ...
  /* 글자로 먼저 뽑는다 — 라벨 읽기와 되돌아갈 길이 모두 이것을 쓴다 */
  const 온글 = 글자만(
  ...
  /* ★ 라벨로 읽는다 (2026-09-04 실제 화면으로 알아낸 길) */
  const 라 = 라벨로읽기(온글);
  if (라 && (라['제목'] || 라['설명'])) {
    /* 설명이 제목과 같은 자료가 많다 — 같으면 설명을 비워 두 번 안 보이게 */
    const 제목 = String(라['제목'] || '').slice(0, 300);
    let 설명 = String(라['설명'] || '').slice(0, 1000);
    if (설명 && 설명 === 제목) 설명 = '';
    return {
      ok: true,
      제목: 제목,
      고유번호: String(라['자료번호'] || ''),
      기관: String(라['출처'] || '').slice(0, 60),
      날짜: (/(\d{4}-\d{2}-\d{2})/.exec(String(라['작성일'] || '')) || [])[1] || '',
      설명: 설명,
      /* 본문은 «라벨로 읽은 것만» 담는다 — 예전에는 차림표·바닥글이 통째로 들어왔다.
         자료 하나가 수십 KB 면 자리가 금방 부푼다. */
      본문: (설명 || 제목).slice(0, 1000),
      첨부: 첨부,
      읽은법: '라벨'
    };
  }

  /* ── 되돌아갈 길 — 라벨이 없는 쪽(모양이 바뀌었을 때) ──
     ⚠ 여기서도 «못 읽었으면 못 읽었다고 말한다». 빈 값을 성공으로 주지 않는다. */
  const 제목2 = 글자만((/<title>([\s\S]*?)<\/title>/i.exec(s) || [])[1] || "")
    .replace(/\s*-\s*한국공인노무사회.*$/, "").trim();
  const 날짜2 = (/(\d{4}-\d{2}-\d{2})/.exec(온글) || [])[1] || "";
  if (!첨부.length && 온글.length < 40) {
    return { ok: false, 까닭: "읽을 것이 없다 — 상세 쪽 모양이 바뀐 듯하다(엿보기로 확인할 것)" };
  }
  /* ⚠ 라벨을 못 찾았다는 것은 «모양이 바뀌었다»는 신호다. 조용히 넘기지 않고 알린다 —
       안 알리면 어느 날부터 제목이 「ilabor 3.0」이 되어도 아무도 모른다. */
  return { ok: true, 제목: 제목2, 고유번호: '', 기관: '', 날짜: 날짜2,
           설명: '', 본문: 온글.slice(0, 1000), 첨부: 첨부,
           읽은법: '되돌아간길', 알림: '라벨(자료번호·제목·설명)을 못 찾았다 — 쪽 모양이 바뀐 듯하다' };
}
...
/* ══════════════════════════════════════════════════════════════════════════
   끊긴 줄은 «다시 걸면» 붙는다 — 자리를 물려받았을 뿐이다
   ══════════════════════════════════════════════════════════════════════════
   실측 2026-09-05. 대표께서 「📥 상세+첨부」를 누르셨는데 첫 걸음에서 막혔다:

       부르지 못했다 — https://www.kcplaa.or.kr/login (UND_ERR_SOCKET)
       other side closed

   ⚠ 그런데 몇 분 전 「엿보기」는 «같은 주소로» 멀쩡히 다녀왔다.
     주소가 틀린 것도, 아이디가 틀린 것도, 상대가 우리를 막은 것도 아니다.

   까닭: node 는 한 번 튼 줄(연결)을 «끊지 않고 두었다가» 다음에 다시 쓴다.
   그런데 상대 서버는 조용한 줄을 저쪽에서 먼저 끊는다. 우리 쪽은 그것을
   모르고 이미 끊긴 줄에 말을 걸고, 그 순간 「other side closed」가 난다.
   ★ 첫 부름(찬 서버)은 새 줄이라 되고, 두 번째부터(더운 서버) 끊긴 줄을
     물려받아 안 되는 것이다 — 우리가 본 그대로다.

   그래서 «줄이 끊긴 까닭»일 때만 잠깐 쉬었다 다시 건다. 새 줄이 열린다.
   ⚠ 아무 때나 다시 걸면 안 된다. 「비밀번호 틀림」이나 「없는 쪽」을 다시
     거는 것은 상대를 두드리는 짓이고, 계정이 잠길 수도 있다.
     여기 적힌 «줄 문제» 목록만 다시 건다. */
const 다시걸까목록 = [
  "UND_ERR_SOCKET",        /* 저쪽이 먼저 끊었다 — 가장 흔하다 */
  "ECONNRESET",            /* 줄이 끊겼다 */
  "ECONNREFUSED",          /* 잠깐 안 받는다 */
  "EPIPE",                 /* 쓰는 도중 끊겼다 */
  "ETIMEDOUT",             /* 기다리다 지쳤다 */
  "UND_ERR_CONNECT_TIMEOUT",
  "UND_ERR_HEADERS_TIMEOUT",
  "UND_ERR_BODY_TIMEOUT",
  "EAI_AGAIN"              /* 이름 풀이가 잠깐 안 됐다 */
];
const 다시걸기 = 3;          /* 처음 한 번 + 다시 두 번 */

function 다시걸까(코드, 속말) {
  const c = String(코드 || "").toUpperCase();
  if (c && 다시걸까목록.indexOf(c) >= 0) return true;
  /* ⚠ 까닭 코드가 «비어 있는» 채로 오는 일이 있다(undici 판에 따라 다르다).
       그때는 속말로 알아본다 — 코드가 없다고 못 고치면 뜻이 없다. */
  const m = String(속말 || "").toLowerCase();
  if (!m) return false;
  return /other side closed|socket hang up|connection closed|premature close|terminated/.test(m);
}

/* 몇 밀리초 쉬었다 다시 걸까 — 갈수록 길게 (0.4초 → 1.2초) */
function 다시걸기쉼(번째) {
  const n = Math.max(1, Number(번째) || 1);
  return Math.min(3000, 400 * n * n);
}

module.exports = {
  벽, 사이트, 브라우저표시,
  다시걸까, 다시걸기쉼, 다시걸기, 다시걸까목록,
  회원사이트, 로그인보내는곳, SSO주소,
  풀기, 글자만, 파일이름,
  막혔나, 로그인됐나, sso주소뽑기, 라벨로읽기, 목록읽기, 쪽수, 첨부읽기, 상세읽기,
