/* ══════════════════════════════════════════════════════════════════
   ...
   ── 왜 index 를 따로 읽나 ──
   회차 원본(rules_mgmt/done/…)에는 신구대조표 «전문»이 들어 있다. 명함첩에서
   회사 하나 보자고 그걸 통째로 받으면 안 된다(명함첩이 카드 전체 대신 경량 색인
   ...
   ── 회사를 맞추는 순서 ──
   사업자번호 › 상호명. 명함첩 ErpMatch 와 같은 규칙으로 다듬는다 —
...
(function (window) {
  ...
  /* 상호명 다듬기 — 법인격 표기와 띄어쓰기·기호를 걷어낸다 (명함첩 ErpMatch 와 같은 규칙) */
  function norm(s) {
