...
(function (global) {
  ...
  function 판례한칸(x) {
    ...
    /* ⚠ 해석은 «사건»이 아니라 «안건»이고, «선고»가 아니라 «회신»이다 (2026-09-07).
         법조 글에서 이름을 잘못 붙이면 그것만으로 우리 신용이 깎인다.
         행정해석에는 법원도 판결유형도 없다 — 그 칸은 저절로 안 그려진다. */
    var 해석인가 = String(x.딱지 || '').indexOf('행정해석') >= 0;
    넣기(해석인가 ? '안건' : '사건', [x.법원, x.사건번호].filter(Boolean).join(' ')
      + (x.사건명 ? ' ' + x.사건명 : ''));
    넣기(해석인가 ? '질의기관' : '사건종류', [x.사건종류, x.판결유형].filter(Boolean).join(' · '));
    넣기(해석인가 ? '회신' : '판결선고', x.선고글 || 날짜꼴(x.선고일));
    넣기('참조조문', x.참조조문);
    var 곁 = 줄들.length
