...
(function (global) {
  ...
  var PROMPT_ALL =
    ...
    /* ── 근로자의 근로계약서 (대표 지시 2026-09-02) ──
       ...
       ⚠⚠ **임금 금액을 담지 않는다.** 급여서류(payslip)와 같은 규칙이다. 까닭이 둘이다 —
         ① 사람마다 다른 민감정보이고, 사진 목록은 화면을 열 때마다 통째로 내려온다.
         ② 계약서 금액은 «틀리게 읽힌 전례»가 있다. 「월 100만원」을 기계가 1,000원으로
            읽어 1만 배 틀린 값이 조용히 들어갈 뻔했다(pu-erp.html 의 erpContractPhotoApplyPatch).
         임금은 원본을 열어 사람이 본다 — 그것이 지금도 하던 일이다.
       ⚠⚠ **주민번호·주소·연락처도 안 담는다.** 근로계약서에는 실제로 다 적혀 있다.
         근로자 서류 넷과 같은 규칙이다 — 필요한 것은 「누구의·언제부터의 계약인가」뿐이다. */
    '\nkind=wcontract 이면 키: name(근로자 이름), company(사업장·회사명), position(직위·직종 — 적혀 있으면), hireDate(근로 시작일 — 2026-09-01 형식), endDate(근로 종료일 — 기간을 정한 계약이면), termType(기간의 정함 — 「있음」 또는 「없음」 중 읽히는 것), docName(문서 제목 그대로 — 예 근로계약서·연봉계약서).' +
    '\n⚠ kind=wcontract 에서 **임금·시급·연봉 금액과 주민등록번호·주소·연락처는 한 글자도 담지 마세요.** pairs 도 담지 마세요.' +
  ...
  var READ_VERSION = 14;  // …/ 11 = 글자 있는 PDF 는 글자로 판독(그림 왕복 없음) / 12 = 통장·계좌(bankbook) 갈래 추가 / 13 = 통장·근로자서류 다섯이 KINDS 에 빠져 other 로 굳던 것을 고침 + 급여명세서 이름 / 14 = 근로계약서(wcontract) 갈래
