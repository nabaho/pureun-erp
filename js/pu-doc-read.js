...
(function (global) {
  ...
  var PROMPT_ALL =
    ...
    '\nkind=form 이면 키: docName(서식 제목 그대로), company(업체·기관명), ceo(대표자), bizno(사업자등록번호), address(주소), companyTel(전화번호), companyFax(팩스번호), name(담당자 이름), title(담당자 직위), mobile(담당자 휴대폰), email(이메일), pairs(그 밖의 **모든** 칸 — 아래 규칙).' +
    '\npairs 규칙: [{"k":"칸 이름 그대로","v":"값 그대로"}] 배열입니다. 서식의 모든 칸을 위에서 아래 순서로 담되, 위 키에 이미 담은 칸은 다시 담지 마세요. 빈 칸은 건너뛰세요. 체크 표시 칸은 v 에 선택된 것을 적으세요(예: "있음 (푸른 노무법인 / 권형하)").' +
  ...
  var READ_VERSION = 7;   // …/ 6 = form(일반 서식) / 7 = 계약서에 위임사무·부가세·상대 연락처
